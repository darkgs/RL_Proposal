...
앞서 설명한 \sdqnname에서 CNN으로부터 추출된 feature는 각각 8x8의 크기를 가지며 96개 존재하였다.
\sdqnname에서는 이를 MLP로 연결하여 각 action에 대한 q값을 계산하였다.
...
\sadqnname은 이런 feature들의 통합을 attention network를 통해 수행함으로써 모델이 상황에 따라 유연하게 feature를 선택적으로 집중할 수 있도록 하며, MLP에 비해 적은 수의 weight를 가지므로 학습에도 좀 더 용이한 구조를 가진다.
우리가 구성한 attention network는 전체적인 상황, context,를 통해 각각 feature의 비중을 구하게 되는데, 기존의 attention이 recurrent neural network의 hidden state를 주로 context를 사용하였지만 우리의 구조에서 그것을 대처할 만한 것을 찾기 힘들다.
